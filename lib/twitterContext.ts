 ...
 */
export async function getRecentTweets(userId, count = 3) {
 ...
 */
export async function getRecentRetweets(userId, count = 2) {
  // モックデータ: 実際の実装ではTwitter APIを呼び出します
  const mockRetweets = [
    {
      id: "5",
      ...
      createdAt: new Date(Date.now() - 5400000).toISOString(), // 1.5時間前
    },
    {
      id: "6",
      originalTweetId: "102",
      ...
      createdAt: new Date(Date.now() - 9000000).toISOString(), // 2.5時間前
    }
  ];
 ...
 */
export async function extractContext(userId) {
  // 最近のツイートとリツイートを取得
